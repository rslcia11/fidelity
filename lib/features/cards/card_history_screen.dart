...
class _CardHistoryScreenState extends State<CardHistoryScreen> {
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      ...
      var rewardsQuery = supabase
          .from('rewards')
          .select('*, businesses(name, reward_description)')
          .eq('loyalty_card_id', widget.loyaltyCardId);

      if (_dateRange != null) {
        final start = _dateRange!.start.toIso8601String();
        final end = _dateRange!.end.add(const Duration(days: 1)).toIso8601String();
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          backgroundColor: Colors.white,
          centerTitle: true,
          title: Text(widget.businessName.toUpperCase(), style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 16, letterSpacing: 1.5)),
          actions: [
            IconButton(
              icon: Icon(_dateRange == null ? Icons.calendar_today_rounded : Icons.calendar_today_rounded, color: _dateRange == null ? Colors.black26 : AppTheme.accentPurple),
  ...
  Widget _buildHistoryList(List<Map<String, dynamic>> items, {required bool isScan}) {
    if (items.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: (isScan ? AppTheme.accentGreen : AppTheme.accentPink).withOpacity(0.05), shape: BoxShape.circle),
              child: Icon(isScan ? Icons.history_rounded : Icons.card_giftcard_rounded, size: 64, color: isScan ? AppTheme.accentGreen : AppTheme.accentPink),
            ),
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final String actionTitle = isScan ? '+1 PUNTO FIDELITY' : 'PREMIO GANADO';
        final accent = isScan ? AppTheme.accentGreen : AppTheme.accentPink;
        
        // Status logic for rewards
        final String status = !isScan ? (item['status'] ?? 'pending') : 'approved';
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: !isScan && status == 'pending' ? statusColor.withOpacity(0.3) : Colors.black.withOpacity(0.04)),
          ),
          child: Column(
            children: [
              Row(
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(color: (isScan ? accent : statusColor).withOpacity(0.1), borderRadius: BorderRadius.circular(16)),
                    child: Icon(isScan ? Icons.add_rounded : Icons.card_giftcard_rounded, color: isScan ? accent : statusColor, size: 24),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(isScan ? actionTitle : (item['reward_description']?.toString().toUpperCase() ?? item['businesses']?['reward_description']?.toString().toUpperCase() ?? actionTitle), style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 13, letterSpacing: 0.5)),
                        const SizedBox(height: 2),
                        Text(
                          EcuadorDateUtils.formatEcuadorTime(date).toUpperCase(),
                          style: const TextStyle(color: Colors.black26, fontSize: 9, fontWeight: FontWeight.w800),
                        ),
                      ],
                    ),
                  ),
              ...
              if (isScan && item['businesses'] != null && item['businesses']['name'] != null) ...[
                const SizedBox(height: 12),
                Row(
                  children: [
                    const Icon(Icons.storefront_rounded, size: 12, color: Colors.black26),
